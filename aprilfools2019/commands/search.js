...
module.exports = {
    ...
    run : async (message, args) => {
        if (args.length === 1 && args[0] === 'help'){
            message.channel.send(morse.encode('This command takes a series of criteria separated by ";" semicolons.\n' +
                ...
                if (prefixes.includes(constraint[0])) {
                    ...
                    if (searchType === 'name') {
                        ...
                        if (factionKey) {
                            ...
                            message.reply(morse.encode(`I didn\'t recognise the faction "${searchString}", so I ignored it`));
                        }
                        ...
                        if  (!isNaN(constraintNum)) {
                            ...
                            message.reply(morse.encode(`I didn\'t recognise the ${searchType} "${searchString}" as a number, so I ignored it`));
                        }
                        ...
                        if(constraint === 'all'){
                            ...
                            if  (!isNaN(constraintNum)) {
                                ...
                                message.reply(morse.encode(`I didn\'t recognise the limit "${searchString}" as a number, so I ignored it`));
                            }
                        }
                    }
                } else {
            ...
            //No Matches
                message.channel.send(morse.encode(`No cards matching your search criteria were found.`));
                return;
            } else if (matchingCards.size < 5){
            //A Few Matches
                message.author.send(morse.encode(`${matchingCards.size} cards were found matching your search criteria.`))
                              .then(()=>{
                                  matchingCards.forEach((card, cardName, map) => {
                                      var cardImageURL = `http://downloads.stormwarsgame.com/cards/${card.URLCardName}.png`;
            ...
            message.channel.send(morse.encode(`Answer sent as DM, search found: ${matchingCards.size} results`));
            return;
        }
    }
}
